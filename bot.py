...
def price_diff_to_pips(diff, is_jpy):
    """Read-only: يحول فرق سعر إلى pips، للتحذيرات فقط. لا تؤثر على أي قرار."""
    if diff is None:
        return None
    return round(diff / 0.01, 1) if is_jpy else round(diff / 0.0001, 1)


def check_warnings(trade):
    """Read-only: يبني لائحة تحذيرات إحصائية بناءً على معطيات محسوبة أصلاً.
    لا يغيّر أي قرار — البوت يبعت الصفقة دايماً بنفس الأزرار ✅/❌."""
    warnings = []
    m15 = trade["details"]["15min"]
    is_jpy = trade["pair"].endswith("JPY") or trade["pair"].startswith("JPY")

    # 1 — SL داخل منطقة FVG (أقوى دليل: 37.5% win)
    fvg_low = m15.get("fvg_low")
    fvg_high = m15.get("fvg_high")
    sl = trade["sl"]
    if fvg_low is not None and fvg_high is not None:
        if min(fvg_low, fvg_high) <= sl <= max(fvg_low, fvg_high):
            warnings.append(("🔴", "SL داخل منطقة FVG (37.5% win تاريخياً)"))

    # 2 — room_pips صغيرة (≤ 5.5 pips)
    room_pips = price_diff_to_pips(trade.get("structural_room"), is_jpy)
    if room_pips is not None and room_pips <= 5.5:
        warnings.append(("🔴", f"مساحة هيكلية ضيقة ({room_pips} pips)"))

    # 3 — Sweep قديم (candles_since_sweep >= 5)
    candles_since_sweep = m15.get("candles_since_sweep")
    if candles_since_sweep is not None and candles_since_sweep >= 5:
        warnings.append(("🟡", f"Sweep قديم ({candles_since_sweep} شموع)"))

    # 4 — شمعة تأكيد ضعيفة (body < 40%)
    o = m15.get("confirmation_open")
    h = m15.get("confirmation_high")
    l = m15.get("confirmation_low")
    c = m15.get("confirmation_close")
    body_pct = None
    if None not in (o, h, l, c):
        candle_range = h - l
        if candle_range > 0:
            body_pct = round((abs(c - o) / candle_range) * 100, 1)
    if body_pct is not None and body_pct < 40:
        warnings.append(("🟡", f"شمعة التأكيد ضعيفة ({body_pct}% body)"))

    # 5 — دخول سريع بعد BOS (< 2 شموع)
    candles_since_bos = m15.get("candles_since_bos")
    if candles_since_bos is not None and candles_since_bos < 2:
        warnings.append(("🟡", f"دخول سريع بعد BOS ({candles_since_bos} شموع)"))
...
def analyze_pair(pair):
    ...
    structural_room = None  # Read-only: للتحذيرات فقط
    ...
    return {
        ...
        "h1_diag": h1_diag,
        "h4_diag": h4_diag,
        "structural_room": structural_room,   # Read-only: للتحذيرات فقط
    }
    ...
    while True:
        ...
        try:
            ...
            for pair in PAIRS:
                ...
                warnings_list = check_warnings(trade)
                warnings_section = ""
                if warnings_list:
                    warnings_section = f"\n⚠️ <b>تحذيرات مكتشفة ({len(warnings_list)}/5):</b>\n"
                    warnings_section += "\n".join([f"  {emoji} {text}" for emoji, text in warnings_list])
                    warnings_section += "\n"
